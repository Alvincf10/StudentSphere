...
@section('content')
    ...
    <div class="col-md-12 p-3 d-flex justify-content-center">
        <div class="card card-payment mt-4 p-4">
            ...
            <strong><h1 class="card-title fs- mb-4 ms-2">Rp.10.000</h1></strong>
            {{-- <strong><h1 class="card-title fs- mb-4 ms-2">{{$programTransaction->price == 0 ? 'Free' : currency_IDR($programTransaction->price)}}</h1></strong> --}}
            ...
            <div class="group-payment">
                <form action="" method="POST">
                    ...
                    {{-- <button type="submit" class="btn btn-primary w-100"><a href="{{route('event.purchase.changeStatus',['id'=>$programTransaction->id])}}" class="text-white"> Payment</a></button> --}}
                </form>
            </div>
        </div>
    </div>
